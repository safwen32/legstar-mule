 ...
 * 
 */
public class LegstarTcpMessageDispatcherTestCase extends
    ...
    public void testInvalidRequest() throws Exception {
        ...
        try {
            ...
            assertTrue(e.getMessage().startsWith(
                    "Mule message body is not a LegStar message."));
        }
    }
    ...
    /**
     * Check that a dispatcher is reusable. Without a keepSendSocketOpen
     * property set to true on the connector, the dispatcher will use 2
     * different sockets for each request.
     * 
     * @throws Exception if test fails
     */
    @Test
    ...
    public void testSocketReuse() throws Exception {
        LegstarTcpMessageDispatcherFactory factory = new LegstarTcpMessageDispatcherFactory();
        OutboundEndpoint endpoint = getEndpoint();
        ((LegstarTcpConnector) endpoint.getConnector())
                .setKeepSendSocketOpen(true);
        MessageDispatcher dispatcher = factory.create(endpoint);
        dispatcher.initialise();
        MuleMessage muleReplyMessage = dispatcher.process(
                getTestEvent(getLsfileaeMessage100())).getMessage();
    ...
    /**
     * Check that if sockets are reused and a timeout occurs on the mainframe (2
     * seconds), the pooling mechanism will create a new socket.
     * 
     * @throws Exception if test fails
     */
    @Test
    ...
    public void testAuthenticationError() throws Exception {
        LegstarTcpMessageDispatcherFactory factory = new LegstarTcpMessageDispatcherFactory();
        OutboundEndpoint endpoint = getEndpoint();
        LegstarTcpConnector connector = (LegstarTcpConnector) endpoint
                .getConnector();
        ...
        dispatcher.initialise();
        try {
            dispatcher.process(getTestEvent(getLsfileaeMessage100()));
            fail();
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(
    ...
    public void testNonExistentProgramError() throws Exception {
        ...
        dispatcher.initialise();
        try {
            dispatcher.process(getTestEvent(getNonExistentProgramMessage100()));
            fail();
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(
                    "CICS command=LINK COMMAREA failed, resp=PGMIDERR, resp2=3",
                    e.getCause().getMessage());
        }
    }

    /**
     * Test data structure mismatch error. This test generates an ASRA abend and
     * a dump which may cause CICS to stress if done too often. For this reason
    ...
    public void optionalTestDataStructureMismatchError() throws Exception {
        ...
        try {
            dispatcher
                    .process(getTestEvent(getDataStructureMismatchMessage100()));
            fail();
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(
                    "No response from host. Connection reset or server transaction abend.",
                    e.getCause().getMessage());
        }
    }
    ...
    public void testNonExistentEndpointError() throws Exception {
        ...
        try {
            ...
            e.printStackTrace();
            assertEquals("Connection refused: connect", e.getCause()
                    .getMessage());
        }
    }
    ...
    private OutboundEndpoint getNonExistentEndpoint() throws Exception {
        EndpointBuilder endpointBuilder = new EndpointURIEndpointBuilder(
                new URIBuilder("legstar-tcp://mainframe:65101", muleContext));
        OutboundEndpoint endpoint = muleContext.getEndpointFactory()
                .getOutboundEndpoint(endpointBuilder);
        LegstarTcpConnector connector = (LegstarTcpConnector) endpoint
                .getConnector();
     ...
     * @throws HostMessageFormatException if message cannot be serialized as a
     *             byte array
     */
    private static byte[] getLsfileaeMessage100() throws HeaderPartException,
            HostMessageFormatException {
        ...
        legstarMessage.addDataPart(new CommareaPart(HostData
                .toByteArray(LsfileaeCases.getHostBytesHexRequest100())));
     ...
     *             byte array
     */
    private static byte[] getNonExistentProgramMessage100()
            throws HeaderPartException, HostMessageFormatException {
        Map < String, Object > map = new HashMap < String, Object >();
    ...
    /**
     * Create a message which data does not match the target program input
     * commarea.
     * 
     * @return a message ready for transport
     * @throws HeaderPartException if LegStar message cannot be created
     ...
     *             byte array
     */
    private static byte[] getDataStructureMismatchMessage100()
            throws HeaderPartException, HostMessageFormatException {
        Map < String, Object > map = new HashMap < String, Object >();
     ...
     */
    private static void checkLsfileae100Reply(final MuleMessage muleReplyMessage)
            throws Exception {
        ...
        assertEquals(1, replyLegStarMessage.getHeaderPart()
                .getDataPartsNumber());
        assertEquals(
                LsfileaeCases.getHostBytesHexReply100(),
                HostData.toHexString(replyLegStarMessage.getDataParts().get(0)
                        .getContent()));
    }
}
